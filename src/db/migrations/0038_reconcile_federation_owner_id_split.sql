-- 0038_reconcile_federation_owner_id_split.sql
...
-- minted a RANDOM UUID for a remote owner agent, while resources and manifest
-- references on this instance are keyed by the origin's EXTERNAL id (see
-- getResourceReferenceKey / resolveActiveResourcePointer). The owner-agent path
-- therefore diverged from the resource/reference path, creating TWO agent rows
-- for one remote actor — a duplicate that split ownership/attribution and broke
-- "my content" / delete gates. resolveLocalEntityId now keys NEW mappings by the
-- external id so the paths converge going forward; this script reconciles legacy
...
-- (this ecosystem applies sovereign migrations manually; chain is idempotent).

-- 1) Repoint resources owned by the diverged random-id stub onto the external-id
--    canonical agent (the id resources/manifest references already key by).
WITH split AS (
...
--    external id (matching the resource/reference key) instead of the legacy
--    random id.
WITH split AS (
  SELECT m.origin_node_id,
         m.external_entity_id
