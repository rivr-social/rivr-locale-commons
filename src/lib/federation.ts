 ...
 * fresh random UUID). Resources and manifest references on this instance are
 * already keyed by the origin's external entity id (see `getResourceReferenceKey`
 * / `resolveActiveResourcePointer`), so minting a random local id for the OWNER
 * agent here produced a SECOND, divergent owner agent for the same remote actor
 * (the H2 owner-id split / duplicate-agent bug). Keying the entity map on the
 * external id converges every path on one row — the placeholder-upgrade path in
 * `ensureProjectedAgent` then enriches the stub in place. Falls back to a random
 * UUID only for the defensive (and not-expected-in-this-ecosystem) case where the
 * external id is not a valid UUID, since `local_entity_id` is a `uuid` column.
 */
