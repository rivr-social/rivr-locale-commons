 ...
 *
 * Connector/connection OAuth access + refresh tokens are secrets. Until now they
 * were persisted as plaintext in `user_connectors` (and will be in
 * `user_connections`). This module wraps AES-256-GCM so those columns hold
 * ciphertext instead, while remaining backward compatible with rows written
 ...
 * Used to distinguish encrypted columns from legacy plaintext.
 */
 ...
 *
 * @param plaintext - The secret to protect (e.g. an OAuth access/refresh token).
