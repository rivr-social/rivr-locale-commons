...
export function requiredVisitorCapability(
  ...
  switch (key) {
    // Light reactions — this app's native legacy mutation type names plus the
    // cross-instance interaction-protocol aliases.
    case "react":
    case "toggleReaction":
    ...
    case "message_thread_start":
      return "message";
    default:
      // Everything else (authoring, vouchers, thanks-token transfers, job
      // applications, group-admin, membership, social-graph) is owner/peer-only
      // regardless of visitor settings, as is any unknown key.
      return null;
  }
}
