...
const SELF = "https://boulder.rivr.social";
...
describe("resolveEntityHref", () => {
  ...
  it("local group + globalFallback:false → verbatim local path, isRemote false (route exists here)", () => {
    const result = resolveEntityHref({}, "/groups/abc", { globalFallback: false });
    expect(result).toEqual({ href: "/groups/abc", isRemote: false });
  });

  it("local profile + globalFallback:false → verbatim local path, isRemote false", () => {
  ...
  it("local ring + globalFallback:true → global aggregator URL, isRemote true (no /rings route here)", () => {
    const result = resolveEntityHref({}, "/rings/abc", { globalFallback: true });
    expect(result).toEqual({ href: "https://global.test/rings/abc", isRemote: true });
  });
  ...
  it("loop guard: a self-host ring stamp with globalFallback:true routes to global, not back to us", () => {
    const result = resolveEntityHref(
      { homeBaseUrl: SELF },
      "/rings/abc",
  ...
  it("a stamp on a DIFFERENT host is respected even when selfBaseUrl is provided", () => {
    ...
    expect(result).toEqual({ href: `${SPIRIT}/rings/abc`, isRemote: true });
  });
