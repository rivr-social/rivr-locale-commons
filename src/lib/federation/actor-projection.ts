...
export async function ensureLocalActorAgent(actorId: string): Promise<void> {
  ...
  await db
    ...
    .values({
      ...
      // authoritative type is reconciled in place when the home peer federates
      // this agent's upsert — the import materializer's placeholder-upgrade path
      // (see `importFederationEvents`, agent branch) sets the real `type` — so an
      // org never stays a person.
      type: "person",
      visibility: "private",
      metadata: {
