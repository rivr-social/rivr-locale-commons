...
async function resolveHomeInstanceWithDepth(
  ...
  if (agent.length > 0) {
    ...
    // stamp (no registered local node and no parent chain) — e.g. a
    // federated-viewer projection minted by `ensureLocalActorAgent`. Consult it
    // so the server-side home guard agrees with the registry API and the client
    // redirect (H1: profile buttons not routing to the sovereign home). Same
    // loop guard as the registry path: never resolve to ourselves.
 ...
 * stamp (the remote-viewer cookie and federated projection stamps carry the
 * URL, not the node id). Returns null when no non-archived node is registered
 * for that URL — common on a sovereign, where a visitor's home is reached
 * through the global hub rather than registered locally.
 *
 * Shared with {@link ensureLocalActorAgent} so projection and home resolution
 * agree on one lookup.
 */
 ...
 *
 * A federated-viewer projection (see `ensureLocalActorAgent`) may carry the
 * home as a URL rather than a registered node id. Prefers the explicit
 * `homeBaseUrl`; otherwise falls back to the origin of `canonicalUrl`.
 *
 * Returns null when no URL stamp is present, when it points back at this
 * instance (loop guard), or when no non-archived node is registered for it.
 */
async function resolveViaHomeBaseUrlMetadata(
  ...
  // ourselves (mirrors the registry path's local-node check).
