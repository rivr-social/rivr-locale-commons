 ...
 *
 * Auth: the instance OWNER only. When a primary agent is configured (the
 *   normal sovereign case) only that agent may administer visitor access;
 *   otherwise fall back to a platform-admin check (`metadata.siteRole === "admin"`).
 *
 * Security:
...
async function requireOwnerOrRespond(): Promise<NextResponse | null> {
  ...
  // On a configured person/group/locale/region instance the owner is the primary agent.
  if (primaryAgentId) {
